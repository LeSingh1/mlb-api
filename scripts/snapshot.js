...
if (games.length === 0) {
  console.log(`${ymd}: no MLB games today; leaving snapshot unchanged`);
  process.exit(0);
}
mkdirSync("data", { recursive: true });
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "baseball", league: "mlb", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} MLB games -> data/latest.json`);
